...
export const US_STATE_BY_CODE = new Map<UsStateCode, (typeof US_STATES)[number]>(
  US_STATES.map((state) => [state.code, state]),
);

export const US_STATE_BY_FIPS = new Map<string, (typeof US_STATES)[number]>(
  US_STATES.map((state) => [state.fips, state]),
);

export function isUsStateCode(value: string): value is UsStateCode {
  return US_STATE_BY_CODE.has(value as UsStateCode);
}
