...
interface IcaFillablePdfViewerProps {
  ...
  actions?: ReactNode;
  /** Fires whenever the visible page changes, including on scroll. */
  onPageChange?: (page: number) => void;
}

const PARSE_TIMEOUT_MS = 30_000;

const ICA_HOST_ID = "ica-pdf-host";
...
const ICA_JUMP_ITEMS = ICA_SECTION_JUMPS.map((section) => ({
  value: String(section.page),
  label: section.label,
  id: `${ICA_HOST_ID}-jump-${section.page}`,
  controls: ICA_HOST_ID,
}));
...
const IcaFillablePdfViewer = forwardRef<IcaFillablePdfViewerHandle, IcaFillablePdfViewerProps>(
  function IcaFillablePdfViewer(
    { className, prefillLegalName = "", prefillEmail = "", actions, onPageChange },
    ref,
  ) {
