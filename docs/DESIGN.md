...
| Signing | Sheet | The old right sidebar. A "Sign" button in the pager opens it once the last page has been read; the ICA gates on page 14, the W-9 does not gate, because its fields are on page 1 and its remaining five pages are IRS instructions. The submit inside it closes the sheet before the unchanged handler runs: a modal `<dialog>` sits in the top layer and a toast cannot paint over it, so a validation message fired from an open sheet would be invisible. |
