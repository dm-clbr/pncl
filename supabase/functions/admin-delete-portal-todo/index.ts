...
serve(async (req) => {
  ...
  try {
    ...
    logOnboarding("admin_portal_todo_deleted", { adminId: user.id, todoId: id });
    return jsonResponse({ id, message: "To-do deleted." });
  } catch (error) {
    if (error instanceof AdminAuthError) {
      return errorResponse(error.message, error.status, error.code);
    }
    const message = error instanceof Error ? error.message : "Unable to delete to-do";
    logOnboarding("admin_delete_portal_todo_failed", { error: message }, "error");
