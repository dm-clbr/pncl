...
function validateReorderPayload(body: unknown): { orderedIds: string[] } {
  ...
  if (!Array.isArray(orderedIds) || orderedIds.some((id) => typeof id !== "string" || !id.trim())) {
    throw new Error("orderedIds must be an array of incentive ids");
  }
...
serve(async (req) => {
  ...
  try {
    ...
    logOnboarding("admin_incentives_reordered", { adminId: user.id, count: orderedIds.length });
    return jsonResponse({ message: "Incentive order updated." });
  } catch (error) {
    if (error instanceof AdminAuthError) {
      return errorResponse(error.message, error.status, error.code);
    }
    const message = error instanceof Error ? error.message : "Unable to reorder incentives";
    logOnboarding("admin_reorder_incentives_failed", { error: message }, "error");
