...
export function getPdfLibFieldName(
  form: { getFields: () => Array<{ getName: () => string }> },
  key: W9FormFieldKey,
): string | null {
  const suffix = W9_FORM_FIELD_SUFFIXES[key];
  const direct = form.getFields().find((field) => field.getName() === suffix);
  if (direct) return direct.getName();

  const match = form.getFields().find((field) => {
    const name = field.getName();
    return name.endsWith(`.${suffix}`) || name.endsWith(suffix);
  });
  return match?.getName() ?? null;
}
