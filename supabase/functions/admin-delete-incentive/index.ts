...
function validateDeletePayload(body: unknown): { id: string } {
  ...
  if (!id) {
    throw new Error("Incentive id is required");
  }
...
serve(async (req) => {
  ...
  try {
    ...
    logOnboarding("admin_incentive_deleted", { adminId: user.id, incentiveId: id });
    return jsonResponse({ id, message: "Incentive deleted." });
  } catch (error) {
    if (error instanceof AdminAuthError) {
      return errorResponse(error.message, error.status, error.code);
    }
    const message = error instanceof Error ? error.message : "Unable to delete incentive";
    logOnboarding("admin_delete_incentive_failed", { error: message }, "error");
