...
serve(async (req) => {
  ...
  try {
    ...
    logOnboarding("admin_carrier_deleted", { adminId: user.id, carrierId: id });
    return jsonResponse({ id, message: "Carrier deleted." });
  } catch (error) {
    if (error instanceof AdminAuthError) {
      return errorResponse(error.message, error.status, error.code);
    }
    const message = error instanceof Error ? error.message : "Unable to delete carrier";
    logOnboarding("admin_delete_carrier_failed", { error: message }, "error");
